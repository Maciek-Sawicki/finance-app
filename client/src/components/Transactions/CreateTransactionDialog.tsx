...
export const CreateTransactionDialog = ({
  ...
  const [form, setForm] = useState<any>({
    ...
    customToAmount: "",
    description: "",
    date: new Date().toISOString().slice(0, 10), // tylko data
    exclude: false, // switch Exclude
    settled: false, // domyślnie false
  });
...
//                 key={t}
//                 value={t}
//                 className={`p-2 rounded ${tab === t ? "bg-primary tx" : "bg-muted"}`}
//               >
//                 {t.charAt(0).toUpperCase() + t.slice(1)}
//               </Tabs.Trigger>
//             ))}
...
//               <Label>Exclude</Label>
//               <Switch checked={form.exclude} onCheckedChange={val => handleChange("exclude", val)} />
//             </div>
//                 </>
//               )}
...
return (
  <Dialog open={open} onOpenChange={onClose}>
    <DialogContent className="max-w-lg bg-[hsl(var(--card))] text-[hsl(var(--card-foreground))]">
      <DialogHeader>
        <DialogTitle>Create Transaction</DialogTitle>
        <DialogDescription>Choose transaction type and fill in details.</DialogDescription>
      </DialogHeader>
