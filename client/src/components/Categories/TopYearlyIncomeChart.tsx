...
export function TopYearlyIncomeChart() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      const res = await CategoriesService.getTopYearlyCategories(userCurrency, "income");
      if (!isMounted) return;
  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden">
      <CardHeader className="pb-4 flex justify-between items-center">
        <div>
          <CardTitle className="text-2xl">Income Breakdown</CardTitle>
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        <div className="flex items-center gap-2 font-medium leading-none">
          Income overview <TrendingUp className="h-4 w-4" />
        </div>
        <div className="leading-none text-muted-foreground">
          Showing top income categories for {selectedYear}
        </div>
      </CardFooter>
    </Card>
  );
